	...
	class Pootle_Update_Manager extends Pootle_Update_Manager_Menu {
		...
		public function __construct( $token, $name, $version, $file, $text_domain = null, $upgrade_url = 'http://pootlepress.com/' ) {
			...
			if ( is_admin() ) {
				...
				$this->renew_license_url = $this->upgrade_url; // URL to renew a license. Trailing slash in the upgrade_url is required.
		 ...
		 * API Key Class.
		 *
		 * @return Pootle_Update_Manager_Key
		 */
		public function set_key() {
			$this->key_class = new Pootle_Update_Manager_Key( $this->product_id, $this->instance_id, $this->software_version, $this->upgrade_url, $this->domain );
		}

		/**
		 * Update Check Class.
		 *
		 * @return Pootle_Update_Manager_Update_Check
		 */
		public function update_check( $upgrade_url, $plugin_name, $product_id, $api_key, $activation_email, $renew_license_url, $instance, $domain, $software_version, $plugin_or_theme, $text_domain, $extra = '' ) {
			...
			return new Pootle_Update_Manager_Update_Check( $upgrade_url, $plugin_name, $product_id, $api_key, $activation_email, $renew_license_url, $instance, $domain, $software_version, $plugin_or_theme, $text_domain, $extra );
		}
